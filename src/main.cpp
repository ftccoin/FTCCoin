...
#if defined(NDEBUG)
# error "FINAD cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "FINAD Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // FTC: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// FTC: find block wanted by given orphan block
...
// FTC: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // FTC: target change every block
    // FTC: retarget with exponential moving toward target spacing
    CBigNum bnNew;
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // FTC: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // FTC: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // FTC: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // FTC: track money supply and mint amount info
...
// FTC: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // FTC: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // FTC: compute stake entropy bit for stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // FTC: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// FinadCoin: attempt to generate suitable proof-of-stake
