...
inline bool IsProtocolV3(int64_t nTime) { return TestNet() || nTime > 1594909808; }
...
public:
    ...
    {
        // FTC: the coin stake transaction is marked with the first output empty
...
public:
    ...
    // FTC: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // FTC: two types of block: proof-of-work or proof-of-stake
    ...
    // FTC: get max transaction timestamp
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // FTC: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // FTC: block index flags
    enum  
    {
