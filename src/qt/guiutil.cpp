...
{
    // FinadCoin: check prefix
    if(uri.scheme() != QString("finad"))
        return false;
...
{
    // Convert finad:// to finad:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("finad://"))
    {
        uri.replace(0, 12, "finad:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FINAD.lnk";
}
...
{
    return GetAutostartDir() / "finad.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FINAD-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  finad-qt [" + tr("command-line options") + "]                     " + "\n";
